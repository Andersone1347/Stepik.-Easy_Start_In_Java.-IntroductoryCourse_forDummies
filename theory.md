...
Многомерный, или вложенный, массив - это массив, каждый элемент которого, в свою очередь, является массивом. В жизни мы постоянно встречаемся с такой структурой данных, простейшим примером служат обыкновенные таблицы.

Действительно, мы можем представить таблицу в виде массива строк, каждая из которых, в свою очередь, является массивом данных. Таким образом, таблица - это двумерный массив.

В Java многомерные массивы определяются аналогично одномерным:
```
int[][] sample = {{1, 2, 3}, {4, 5, 6}};
```
Мы объявили массив, который имеет два элемента: {1, 2, 3} и {4, 5, 6}, каждый из который также является массивом.

Доступ к элементам в многомерном массиве осуществляется аналогичным способом. Сначала мы указываем индекс элемента первого уровня (который сам является массивом), а затем - индекс элемента второго уровня, также в квадратных скобках.
```
int[][] sample = {{1, 2, 3}, {4, 5, 6}};
...
```
Индексация, как обычно, начинается с нуля, а если указать неверный индекс - программа выдаст ошибку.
...
```
В примере определён двумерный массив (таблица), состоящий из трёх "строк", при этом в первой "строке" - три "ячейки", во второй - одна, а в третьей - четыре.
...
```
Java не ограничивает нас двумерными массивами - уровней вложенности может быть, сколько угодно. Однако нужно помнить, что управлять многомерными массивами очень сложно.

**Матрицы**

Двумерный массив размерами m*n называется матрицей. Все строки матрицы имеют одинаковое количество элементов, то же самое относится и к столбцам. Грубо говоря, матрица - это прямоугольный объект (таблица), в котором на пересечении строк и столбцов расположены элементы. Традиционно первая размерность (m) - это количество строк в матрице, вторая (n) - количество столбцов. Таким образом, элемент матрицы 
x(ij)​ расположен в строке с индексом i и в столбце с индексом j.    
[alt](/img/%D0%9C%D0%B0%D1%82%D1%80%D0%B8%D1%86%D0%B0.jpg "матрица")   
